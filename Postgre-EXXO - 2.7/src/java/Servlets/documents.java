...
//        String role=(String)session.getAttribute("role");
        ...
        String fio = "ВИРТУАЛЬНАЯ ФАЙЛОВАЯ СИСТЕМА";
//             Vector comp = new <Vector> Vector();
        ...
        try {
            ...
            String sql = "SELECT id, name, file, superior, type, fname, id<>copy AS isCopy, step FROM files WHERE owner='" + owner + "' ORDER BY file, name DESC";
                    ...
                    while (!st1.empty()) {
                        ...
                        if (v.get(3) != null) {
                            if (v.get(3).equals(marker)) {
                                ...
                                String type = "";
                                String copy = "";
                                if (v.get(6).equals(" copy")) {
                                    copy = " (копия)";
                                }
                                if (v.get(2).equals("fil")) {
                                    center += copy + " <span class=\"filepath\">(" + v.get(5) + ")</span>";
                                    String t = v.get(4);
                                    type = EXXOlib.DOCTYPES.getType(t, v.get(5));
                                }

                                String title = "<div class=\"" + v.get(2) + " item " + type + v.get(6) + v.get(7) + "\" id=\"" + v.get(0) + "\">";
                                String titleEnd = "</div>";
