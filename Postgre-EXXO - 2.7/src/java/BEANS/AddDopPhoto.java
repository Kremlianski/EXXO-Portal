...
public class AddDopPhoto {
    ...
    public String getList(HttpServletRequest req) throws ClassNotFoundException, SQLException, IOException {
        ...
        if (owner == null) {
            owner = (String) req.getSession(true).getAttribute("id");
        }
        if (s == null) {
            s = "files";
        }
        if (id == null) {
            ...
            while (rs.next()) {
                list += "<a href='addDopusk?dopusk=" + rs.getString("global_id") + "&id=" + id + "&owner=" + owner + "&p=photo' id='" + rs.getString("global_id") + "'><img class='photo' src='IMG?id=" + rs.getString("id") + "' title='" + rs.getString("fio") + ", " + rs.getString("unit") + "'></a>";
