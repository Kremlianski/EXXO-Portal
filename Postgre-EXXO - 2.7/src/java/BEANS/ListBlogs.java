...
public class ListBlogs {
    ...
    public String getList(HttpServletRequest r) throws ClassNotFoundException, SQLException {
        ...
        CallableStatement cstmt = con.prepareCall("{? = call ListBlogs(?, ?)}");
        cstmt.registerOutParameter(1, Types.ARRAY);
        cstmt.setString(2, ROWS);
        cstmt.setString(3, offset);
        cstmt.executeUpdate();
        ...
        if (of > 0) {
            ...
            navi += "<a href='listBlogs.jsp?of=" + prev + "' class='exxo-shadow' id='backward'></a>";
        }
        navi += "</td><td></td><td class='threeNavi'>";
        if (next) {
            int sled = of + rows;
            navi += "<a href='listBlogs.jsp?of=" + sled + "'  class='exxo-shadow' id='forward'></a>";
        }
