...
public class NewBlogs {
    ...
    public String getList(HttpServletRequest r, String owner, String t, String guser) throws ClassNotFoundException, SQLException {
        ...
        CallableStatement cstmt = con.prepareCall("{? = call NewBlogs(?,?,?,?,?)}");
        ...
        if (of > 0) {
            ...
            navi += "<a href='newBlogs.jsp?of=" + prev + andown + "' class='exxo-shadow' id='backward'></a>";
        }
        navi += "</td><td></td><td class='threeNavi'>";
        if (next) {
            int sled = of + rows;
            navi += "<a href='newBlogs.jsp?of=" + sled + andown + "'  class='exxo-shadow' id='forward'></a>";
        }
