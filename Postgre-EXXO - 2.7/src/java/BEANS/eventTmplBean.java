...
public void initParams(String dateString) throws ClassNotFoundException, SQLException {
 ...
 //  SimpleDateFormat sf1=new SimpleDateFormat("EEEE, dd MMMM, yyyy");
   ...
   String sql="SELECT hours, minutes, day, name, long, id, extract(year from day) AS Year, extract(month from day) AS Month,extract(day from day) AS D, descr, important, outside, nophone, hours>=extract(hour from now()) AS actualh, day>=now() AS actualD, extract(day from day)= extract(day from now()) AS equalD    FROM events WHERE day='"+dateString+"' AND user_id='"+user+"'  ORDER BY hours, minutes";
   if(dateString==null||dateString.equals("")) sql="SELECT hours, minutes, day, name, long, id, extract(year from day) AS Year, extract(month from day) AS Month,extract(day from day) AS D, descr, important, outside, nophone , hours>=extract(hour from now()) AS actualh,, day>=now() AS actualD, extract(day from day)= extract(day from now()) AS equalD   FROM events WHERE day=now()::date AND user_id='"+user+"'  ORDER BY hours, minutes";
   startSQL();
   ...
   while(rs.next())
   {
      ...
      event.name="<a href='event.xhtml?id="+rs.getString("id")+"'>"+rs.getString("name")+"</a>";
      ...
      event.important=rs.getBoolean("important");
      event.nophone=rs.getBoolean("nophone");
      event.outside=rs.getBoolean("outside");
      event.file="<a href='docPoket.jsp?ev="+event.id+"&amp;s=docEvent' class='file'>Файлы:</a>";
      event.kill="<a href='eventsAdd.xhtml?id="+event.id+"' class='kill'>редактировать</a>";
      add="<a href=\"eventsAdd.xhtml?date="+event.date+"\" id=\"new\">новое событие</a>";
      event.list="";
      ...
      if(rs.getBoolean("actualD")||(rs.getBoolean("equalD")&&rs.getBoolean("actualH"))) event.actual=true;
      if(!event.actual)event.data="<span class='notactual'>"+event.data+"</span>";
     sql="SELECT * FROM docsEvent WHERE docsEvent.id='"+event.id+"'";
     ResultSet rs1=stmt1.executeQuery(sql);
     if(rs1.next()){event.files=true;}
     rs1.close();
   ...
   con.close();
    ...
    if(list.contains(i)) sb.append("busy ");
    else sb.append("free ");
    if(i==0||i%4==0) sb.append("hour");
    sb.append("'");
...
public String getTimedown(){return timedown;}
public String getTimeup(){return timeup;}
...
public String getSchedule(){
    ...
    for(int i=9; i<=24;i++){
    for(int y=0;y<4;y++){
    if (y==3) sb.append("<span class='graf'></span>");
    else sb.append("<span></span>");
    }}
    return sb.toString();
...
public void setName(String l){name=l;}
public String getName(){return name;}
public void setList(String l){list=l;}
public String getList(){return list;}
public void setData(String l){data=l;}
public String getData(){return data;}
public void setAdd(String l){add=l;}
public String getAdd(){return add;}
public void setFile(String l){file=l;}
public String getFile(){return file;}
public void setKill(String l){kill=l;}
public String getKill(){return kill;}
public void setId(String l){id=l;}
public String getDescr(){return descr;}
public void setDescr(String l){descr=l;}
public int getLon(){return lon;}
public void setLon(int l){lon=l;}
public String getDate(){return date;}
public void setDate(String l){date=l;}
public String getTime(){return time;}
public void setTime(String l){time=l;}
public String getUser(){return user;}
public void setUser(String l){user=l;}
public boolean getImportant(){return important;}
public void setImportant(boolean l){important=l;}
public boolean getOutside(){return outside;}
public void setOutside(boolean l){outside=l;}
public boolean getNophone(){return nophone;}
public void setNophone(boolean l){nophone=l;}
public void setFilesNo(String l){filesNo=l;}
public String getFilesNo(){return filesNo;}
public boolean getActual(){return actual;}
public void setActual(boolean l){actual=l;}
public boolean getFiles(){return files;}
}
}
