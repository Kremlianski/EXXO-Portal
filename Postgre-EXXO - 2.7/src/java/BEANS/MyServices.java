...
public class MyServices {
    ...
    public String getList(String id, HttpServletRequest r) throws ClassNotFoundException, SQLException {
        ...
        int rows = ROWS + 1;
        String sql = "SELECT pjornal.timeEnter, pjornal.id, process.name, pjornal.status, employee.fio FROM pjornal, process, employee WHERE pjornal.pid=process.id AND process.owner='" + id + "' "
        ...
        if (of > 0) {
            ...
            navi += "<a href='myServices.jsp?of=" + prev + andown + "' class='exxo-shadow' id='backward'></a>";
        }
        navi += "</td><td></td><td class='threeNavi'>";
        if (next) {
            int sled = of + ROWS;
            navi += "<a href='myServices.jsp?of=" + sled + andown + "'  class='exxo-shadow' id='forward'></a>";
        }
