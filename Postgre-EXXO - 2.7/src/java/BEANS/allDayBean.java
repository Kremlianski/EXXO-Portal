...
public class allDayBean {
    ...
    public void initParams(String dateString) throws ClassNotFoundException, SQLException {
        ...
        String sql = "SELECT hours, minutes, day, name, long, id, extract(year from day) AS Year, extract(month from day) AS Month,extract(day from day) AS D, descr, important, outside, nophone, hours>=extract(hour from now()) AS actualh,extract(day from day)> extract(day from now()) actualD, extract(day from day)= extract(day from now()) AS equalD    FROM events WHERE day='" + dateString + "' AND user_id='" + user + "'  ORDER BY hours, minutes";
        if (dateString == null || dateString.equals("")) {
            sql = "SELECT hours, minutes, day, name, long, id, extract(year from day) AS Year, extract(month from day) AS Month,extract(day from day) AS D, descr, important, outside, nophone , hours>=extract(hour from now()) AS actualh,extract(day from day)> extract(day from now()) actualD, extract(day from day)= extract(day from now()) AS equalD   FROM events WHERE day=now()::date AND user_id='" + user + "'  ORDER BY hours, minutes";
        }
        startSQL();
        ...
        while (rs.next()) {
            ...
            sql = "SELECT name, type, fname, files.id AS id FROM files, docsEvent WHERE files.id=docsEvent.doc AND docsEvent.id='" + event.id + "'";
            ResultSet rs1 = stmt1.executeQuery(sql);

            while (rs1.next()) {
                if (rs1.isFirst()) {
                    event.filesNo = "";
                    event.list += "<div class='table-about'>";
                }
                if (rs1.isLast()) {
                    event.list += "<a href='fileLoader.jsp?id=" + rs1.getString("id") + "'>" + rs1.getString("name") + "</a><span class='fname'>(" + rs1.getString("fname") + ")</span></div>";
                } else {
                    event.list += "<a href='fileLoader.jsp?id=" + rs1.getString("id") + "'>" + rs1.getString("name") + "</a><span class='fname'>(" + rs1.getString("fname") + "),</span> ";
                }
